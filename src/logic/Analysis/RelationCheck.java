...
public abstract class RelationCheck implements RelationInformation {
   ...
   */
  public int getNumberOfMatchingAttributes(ArrayList<Attribute> list1,
   ...
   * @param candidateKeys
   *          the candidate Keys in which to look for the attribute
   * @return true if it is a prime attribute, false if not
   */
  public boolean isPrimeAttribute(Attribute attribute,
   ...
   * 
   * @param fds
   *          functional dependencies to check
   ...
   * 
   * @param fd
   *          the FunctionalDependency to operate on
   * @return the fd if it was cannonical, or cannonical fd's
   */
  public ArrayList<FunctionalDependency> makeFdCannonical(
	...
	if (fd.getTargetAttributes().size() > 1) {
	  ...
	  for (Attribute targetAttribute : fd.getTargetAttributes()) {
		...
		cannonicalFds.add(new FunctionalDependency(fd.getSourceAttributes(),
		    tempAttributes));
	  }
   ...
   * 
   * @param fdList
   *          ArrayList of FunctionalDependencies to work with
   * @return a ArrayList of optimized FunctionalDependencies
   */
  public ArrayList<FunctionalDependency> removeUnneccessarySourceAttributes(
	  ArrayList<FunctionalDependency> fdList) {
   ...
   */
  private void removeObsoleteLeftAttributes(FunctionalDependency fd,
	  ArrayList<FunctionalDependency> fds) {
	...
	for (Attribute attribute : fd.getSourceAttributes()) {
	  ...
	  if (getClosure(fds, testFd.getSourceAttributes()).containsAll(
		  testFd.getTargetAttributes())) {
   ...
   * 
   * @param fds
   *          ArrayList to clean up
   */
  @SuppressWarnings("unchecked")
  public void removeRedundantFunctionalDependencies(
	  ArrayList<FunctionalDependency> fds) {
	...
	for (FunctionalDependency fd : fds) {
	  ...
	  if (getClosure(allFds, fd.getSourceAttributes()).containsAll(
   ...
   * 
   * @param attributes
   *          the initial ArrayList of Attributes
   * @param resultList
   *          a empty ArrayList of ArrayLists
   ...
   * 
   * @param listToTest
   *          the list with the values to search for
   * @param targetList
   *          the list to search in
   * @return true/false if ArrayList already exists / not exists
   */
  private boolean isListAlreadyPresent(ArrayList<Attribute> listToTest,
	  ArrayList<ArrayList<Attribute>> targetList) {
   ...
   * 
   * @param schema
   *          schema which contains the Attributes
  ...
  public ArrayList<Key> getAllCandidateKeys(RelationSchema schema) {
	...
	if (isKeyDeterminingEverything(schema.getAttributes(),
	    schema.getFunctionalDependencies(), new Key(rootNode))) {
   ...
   *          to work with
   * @param m
   *          the Attributes that occur on the left and right side of
   *          fds
   * @param candidateKeys
   *          a empty ArrayList
   * @param level
   *          a set of ArrayLists containing Attributes
   * @return all candidate Keys
   */
  private ArrayList<Key> searchCandidateKeys(RelationSchema schema,
	  ArrayList<Attribute> m, ArrayList<Key> candidateKeys,
	  ArrayList<ArrayList<Attribute>> level) {
	ArrayList<ArrayList<Attribute>> nextLevel = new ArrayList<>();
	ArrayList<Attribute> tempList;

	for (ArrayList<Attribute> attrList : level) {
	  if (isKeyDeterminingEverything(schema.getAttributes(),
		  schema.getFunctionalDependencies(), new Key(attrList))) {
	...
	for (ArrayList<Attribute> attrLst : level) {
	  if (!isCandidateKey(attrLst, candidateKeys)
		  && !isSuperKey(attrLst, candidateKeys)) {
   ...
   * @param fds
   *          fds in which to look for the attribute
   * @return a AttributePosition Object, indicating the position
   */
  public AttributePosition getAttributePosition(Attribute attribute,
	  ArrayList<FunctionalDependency> fds) {
   ...
   *          the key to test
   * @param candidateKeys
   *          the candidateKeys to test against
   * @return true if it is a superKey, false if not
   */
  public boolean isSuperKey(ArrayList<Attribute> keyToTest,
	  ArrayList<Key> candidateKeys) {
	for (Key key : candidateKeys) {
  ...
  public boolean isKeyDeterminingEverything(RelationSchema schema, Key key) {
	return isKeyDeterminingEverything(schema.getAttributes(),
	    schema.getFunctionalDependencies(), key);
  }
   ...
   * 
   * @param attributes
   *          attributes that must be determined
   * @param fds
   *          functional dependencies to consider
   * @param key
   *          key to be tested
   * @return true/false if key determines everything / or not
   */
  public boolean isKeyDeterminingEverything(ArrayList<Attribute> attributes,
	  ArrayList<FunctionalDependency> fds, Key key) {
  ...
  public boolean isMember(RelationSchema schema, FunctionalDependency fd) {
	...
	ArrayList<Attribute> closureOfX = getClosure(schema,
   ...
   * @param schema
   *          schema to work with
   * @param determiningAttributes
   *          Closure(determiningAttributes)
   ...
   *          functional dependencies to consider
   * @param determiningAttributes
   *          determining attributes to work with
  ...
  public boolean areFdSetsEquivalent(ArrayList<FunctionalDependency> list1,
	...
	for (FunctionalDependency fd : list1) {
	  if (!list2.contains(fd)) {
		if (!getClosure(list2, fd.getSourceAttributes()).containsAll(
		    fd.getTargetAttributes())) {
		  return false;
		}
	  }
	...
	for (FunctionalDependency fd : list2) {
	  if (!list1.contains(fd)) {
		if (!getClosure(list1, fd.getSourceAttributes()).containsAll(
		    fd.getTargetAttributes())) {
		  return false;
		}
	  }
   ...
   */
  public void copyAttributes(ArrayList<Attribute> sourceList,
