...
public abstract class Synthese implements NormalizationAlgorithm {
   ...
   * 
   * @param groups
   *          the groups of functional dependencies to work with
   */
  protected void uniteMutualDepdendentGroups(
   ...
   * 
   * @param groups
   *          the overall list of FunctionalDependencies to work with
	  ...
	  for (FunctionalDependency fd : group) {
		for (ArrayList<FunctionalDependency> group2 : groups) {
		  ...
		  for (FunctionalDependency fd2 : group2) {
			if (fd.getTargetAttributes().equals(fd2.getSourceAttributes())
			    && fd2.getTargetAttributes().equals(fd.getSourceAttributes())) {
   ...
   * 
   * @param schemata
   *          all relationSchemata to work with
   * @return the fully contained RelationSchema or null if none
   *         existing
   */
  protected RelationSchema getFullyContainedRelation(
	  ArrayList<RelationSchema> schemata) {
	// TODO: Rework me!
   ...
   * 
   * @param fds
   *          the list of functional dependencies to work with
   ...
   * 
   * @param schemata
   *          the ArrayList of Relations to search in
   * @param candidateKeys
   *          the candidateKeys to look for in the relations
   * @return true if any Relation contains a key, false if not
   */
  protected boolean isAnyRelationContainingAKey(
	  ArrayList<RelationSchema> schemata, ArrayList<Key> candidateKeys) {
	for (RelationSchema schema : schemata) {
   ...
   * 
   * @param fds
   *          the set of functional dependencies to work with
   * @return a ArrayList of Groups having the same left side
   */
  protected ArrayList<ArrayList<FunctionalDependency>> getGroupsWithSameLeftSide(
   ...
   *          the fd to decide on
   * @param list
   *          the list of fds to compare with
   * @return true if it has the same left side, false if not
   */
  private boolean hasSameLeftSide(FunctionalDependency fd,
	  ArrayList<FunctionalDependency> list) {
	for (FunctionalDependency fdep : list) {
	  if (fdep.getSourceAttributes().containsAll(fd.getSourceAttributes())
		  && fd.getSourceAttributes().containsAll(fdep.getSourceAttributes())) {
		return true;
	  }
	}
	return false;
   ...
   * 
   * @param schema1
   *          first relation to compare
   * @param schema2
   *          second relation to compare
   * @return the number of matching functional dependencies
   */
  protected int getNumberOfMatchingFds(RelationSchema schema1,
   ...
   * 
   * @param relations
   *          the relations to work with
   * @param foreignKeys
   *          the foreign keys to take into account
   */
  public void updatePrimaryAndForeignKeys(ArrayList<RelationSchema> relations,
	  ...
	  for (RelationSchema subRelation : relations) {
		...
		for (Attribute attr : relation.getAttributes()) {
		  for (Attribute subAttr : subRelation.getAttributes()) {
			if (attr.getName().equals(subAttr.getName())
			    && subAttr.getIsPrimaryKey()
			    && RelationUtils.getInstance().getKey(subRelation)
			        .getAttributes().size() == 1) {
			  foreignKeys.add(new ForeignKeyConstraint(relation.getName(), attr
				  .getName(), subRelation.getName(), subAttr.getName()));
