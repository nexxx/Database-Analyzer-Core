...
public class Database extends HistoricObject {
   ...
   *          the parentrelation of the attribute to rename
   * @param attribute
   *          the attribute to rename
   * @param newName
   *          the name name for the attribute
   */
   ...
   *          the relation to replace
   * @param sourceRelation
   *          the new relation
   * @return true ==> success, false ==> fail
   */
  public boolean replaceRelationSchema(RelationSchema targetRelation,
   ...
   *          the relation to replace
   * @param sourceRelations
   *          the results of a normalization
   * @param foreignKeys
   *          the new foreignKeys
   * @return true for success, false ==> targetRelation not part of
   *         the db
   */
  public boolean insertNormalizationResult(RelationSchema targetRelation,
	  ArrayList<RelationSchema> sourceRelations,
   ...
   * 
   * @param targetRelation
   *          the old relation that gets replaced
   * @param sourceRelations
   *          the new relations that replace the old relation
   */
  private void updateForeignKeyReferences(RelationSchema targetRelation,
	...
	for (ForeignKeyConstraint fk : foreignKeys) {
	  ...
	  if (fk.getTargetRelationName().equals(targetRelation.getName())) {
		fk.setTargetRelationName(getNewRelationName(fk.getTargetRelationName(),
   ...
   * 
   * @param oldRelationName
   *          the name of the old relation
   * @param attributeName
   ...
   */
  private String getNewRelationName(String oldRelationName,
	  String attributeName, ArrayList<RelationSchema> relations) {
   ...
   *          the new relation
   */
  public void replaceRangeOfRelationsWithoutFiring(
	  RelationSchema targetRelation, ArrayList<RelationSchema> sourceRelations) {
	targetRelation.removeChangeListener(changeListener);
	int index = database.indexOf(targetRelation);
	database.remove(index);
	for (RelationSchema rel : sourceRelations) {
   ...
   * 
   * @param schema
   *          the relation to remove from the database
   * @return true ==> success, false ==> fail
   */
   ...
   * 
   * @param text
   *          search for text as Index or as Name if no number
   ...
   * 
   * @param name
   *          the name of the Relation to look for
   * @return the RelationSchema you are looking for, or null if not
   */
   ...
   */
  public void removeForeignKey(String sourceRelationName,
   ...
   * 
   * @param relationName
   *          the relatioName to look for
   * @param attributeName
   *          the old name of the attribute inside the relation
